...
* 连续碰撞检测 (CCD): 使用一元二次方程根的判别式  求解碰撞时间，杜绝高速穿模。
...
* Continuous Collision Detection (CCD): Solving  using the quadratic formula to prevent "tunneling" at high velocities.
...
直接在浏览器打开 `2D.html`。
Open `2D.html` directly in your browser.

### Desktop Version (Source)

1. 使用 CMake 编译 `physics_core.cpp` 生成 `.dll`。
2. 运行 `main.py`。
3. Compile `physics_core.cpp` using CMake to generate the `.dll`.
4. Execute `main.py`.

### Download EXE (Stable)

请前往 [Releases]下载打包好的可执行文件。
Please go to the [Releases]page to download the pre-compiled binaries.
